...
function generateDrizzleTable(name: string, def: TableDefinition): string {
...
function generateDrizzleSchema(tables: Record<string, TableDefinition>): string {
	const imports = new Set<string>(["pgTable", "index", "uniqueIndex"]);
	...
	for (const [name, def] of Object.entries(tables)) {
		lines.push(generateDrizzleTable(name, def));
		lines.push("");
	}
...
function generatePrismaModel(name: string, def: TableDefinition): string {
	const tableName = toSnakeCase(name);
	const lines: string[] = [];
...
function generatePrismaSchema(tables: Record<string, TableDefinition>): string {
	const lines: string[] = [
		"// Auto-generated by summa generate — do not edit manually",
		"",
		"generator client {",
	...
	for (const [name, def] of Object.entries(tables)) {
		lines.push(generatePrismaModel(name, def));
		lines.push("");
	}
...
function generateKyselySchema(tables: Record<string, TableDefinition>): string {
	...
	for (const name of Object.keys(tables)) {
		lines.push(`  ${toSnakeCase(name)}: ${capitalize(name)}Table;`);
	}
	lines.push("}");
...
export const generateCommand = new Command("generate")
	...
	.action(async (options: { adapter?: string; out?: string; yes?: boolean }) => {
		...
		// Try to load config for plugin schemas
		...
		switch (adapter) {
			case "drizzle":
				output = generateDrizzleSchema(tables);
				defaultFile = "summa.schema.ts";
				break;
			case "prisma":
				output = generatePrismaSchema(tables);
				defaultFile = "summa.prisma";
				break;
			case "kysely":
				output = generateKyselySchema(tables);
				defaultFile = "summa.schema.ts";
				break;
			default:
