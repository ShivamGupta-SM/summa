...
<Callout type="info" title="Need to bypass limits?">
  For chargebacks, network-initiated adjustments, or regulatory debits that must succeed regardless of balance, use [`forceDebit`](/docs/transactions#force-debit) instead. It skips all balance and overdraft checks.
</Callout>
