...
The `@summa-ledger/client` package provides a **type-safe HTTP client** for consuming the Summa ledger API from other services. It mirrors the entire API surface with full TypeScript types, request/response interceptors, and configurable timeouts.
...
|--------|-----------|-------------|
| **Your App** (`@summa-ledger/client`) | HTTP (JSON) | **Summa API** (Hono / Express) |
...
```bash
pnpm add @summa-ledger/client @summa-ledger/core
```
...
client.transactions.correct(params)       // POST /transactions/correct
client.transactions.adjust(params)        // POST /transactions/adjust
client.transactions.journal(params)       // POST /transactions/journal
...
client.holds.createMultiDestination(params) // POST /holds/multi-destination
...
<Callout type="info" title="Typed errors">
  The client throws `SummaClientError` for all API errors, with `code`, `status`, `message`, and `details` fields.
</Callout>

```ts
import { SummaClientError } from "@summa-ledger/client";
...
try {
  ...
  if (error instanceof SummaClientError) {
    ...
    console.log(error.details); // { available: 1000, requested: 5000 }
  }
}
```
...
For advanced use cases, the SDK also exports the internal `FetchClient` with typed `get`, `post`, `put`, and `del` methods:

```ts
import { createFetchClient } from "@summa-ledger/client/fetch";
...
const accounts = await http.get<{ accounts: Account[] }>("/accounts", { status: "active" });
const txn = await http.post<Transaction>("/transactions/credit", { holderId: "user_123", amount: 5000, reference: "dep-001" });
```

| Method | Signature | Description |
|--------|-----------|-------------|
| `get` | `get<T>(path, query?)` | GET request with optional query params |
| `post` | `post<T>(path, body?)` | POST request with JSON body |
| `put` | `put<T>(path, body?)` | PUT request with JSON body |
| `del` | `del<T>(path, body?)` | DELETE request with optional JSON body |

The `FetchClient` applies the same interceptors, timeout, and error parsing as the main client.
...
```bash
pnpm add @summa-ledger/client react
```

```tsx
import { SummaProvider, useSumma, useSummaQuery, useSummaMutation } from "@summa-ledger/client/react";
...
```bash
pnpm add @summa-ledger/client vue
```

```vue
<script setup>
import { provideSumma, useSummaQuery, useSummaMutation } from "@summa-ledger/client/vue";
...
```bash
pnpm add @summa-ledger/client svelte
```

```svelte
<script>
import { createSummaStore } from "@summa-ledger/client/svelte";
...
|--------|---------|-------------|
| `createSummaStore(options)` | `SummaStore` | Create a store factory with `{ client, query, mutation }` |
| `query(fn)` | `SummaQueryStore<T>` | Readable store `{ data, error, loading }` + `refetch()` |
| `mutation(fn)` | `SummaMutationStore<T, V>` | Readable store `{ data, error, loading }` + `trigger(vars)` |
| `SummaStore` | interface | Type for the store factory object |
| `SummaQueryStore<T>` | interface | Readable store extending Svelte `Readable` |
| `SummaMutationStore<T, V>` | interface | Readable store extending Svelte `Readable` |
