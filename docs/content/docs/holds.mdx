...
Holds with `expiresInMinutes` automatically become non-committable after expiration. Hold expiry is a **core worker** that runs automatically when you call `summa.workers.start()` — no plugin needed. The interval is configurable via `coreWorkers`:

```ts
const summa = createSumma({
  database: adapter,
  coreWorkers: {
    holdExpiry: { interval: "2m" },  // Default: "5m"
  },
});
```

You can also void expired holds manually:
