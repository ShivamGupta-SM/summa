...
| **Plugin System** | `audit` · `reconciliation` · `snapshots` · `velocity` · `freezeExpiry` · `outbox` · `dlq` · `hot` · `scheduled` · `admin` · `openApi` · `observability` · `fx` · `accrual` · `reporting` · `retention` · `i18n` · `mcp` · `statements` · `periodClose` · `glSubLedger` · `approvalWorkflow` · `batchImport` · `batchEngine` · `identity` · `apiKeys` · `balanceMonitor` · `backup` · `search` · `dataRetention` |
