...
| **Plugin System** | `audit` · `reconciliation` · `snapshots` · `velocity` · `holdExpiry` · `outbox` · `dlq` · `hot` · `scheduled` · `mnt` · `admin` · `openApi` · `observability` |
...
<Cards>
  ...
  <Card title="14 Built-in Plugins" href="/docs/plugins">
    Reconciliation, snapshots, velocity limits, audit log, outbox, DLQ, hot accounts, scheduled transactions, admin, OpenAPI, observability, statements, and more.
  </Card>
...
| `@summa/telemetry` | Anonymous telemetry for CLI usage analytics |
| `@summa/test-utils` | Test assertions for balance, double-entry, and hash chain verification |
...
<Cards>
  ...
  <Card title="Plugins" href="/docs/plugins">
    Extend with 14 built-in plugins.
  </Card>
