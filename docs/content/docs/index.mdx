...
| **High-traffic accounts** | Hot account pattern, mega CTE combined writes, and TigerBeetle-inspired transaction batching for 100,000+ TPS |
...
| **Multi-tenancy** | Ledger-as-namespace isolation — each tenant gets its own accounts, transactions, and system accounts |
...
| **Summa API** | `ledgers` · `accounts` · `transactions` · `holds` · `events` · `limits` · `workers` |
| **Core Workers** | `holdExpiry` · `idempotencyCleanup` · `leaseCleanup` |
| **Plugin System** | `audit` · `reconciliation` · `snapshots` · `velocity` · `freezeExpiry` · `outbox` · `dlq` · `hot` · `scheduled` · `admin` · `openApi` · `observability` · `fx` · `accrual` · `reporting` · `retention` · `i18n` · `mcp` · `statements` · `periodClose` · `glSubLedger` · `approvalWorkflow` · `batchImport` · `batchEngine` · `identity` · `apiKeys` · `balanceMonitor` · `backup` · `search` · `dataRetention` · `versionRetention` |
...
<Cards>
  ...
  <Card title="30 Built-in Plugins" href="/docs/plugins">
    Audit log, reconciliation, snapshots, velocity limits, FX engine, accrual accounting, financial reporting, transaction batching, identity management, API keys, and more.
  </Card>
  ...
  <Card title="Multi-Tenancy" href="/docs/multi-tenancy">
    Ledger-as-namespace isolation. Each tenant gets its own accounts, system accounts, and event chains.
  </Card>
  ...
  <Card title="100,000+ TPS" href="/docs/scaling">
    Mega CTE combined writes, TigerBeetle-inspired transaction batching, denormalized balance cache, and lock retry with backoff. Full HMAC integrity at every throughput level.
  </Card>
</Cards>
...
<Files>
  <File name="@summa-ledger/summa" />
  <File name="@summa-ledger/core" />
  <File name="@summa-ledger/cli" />
  <File name="@summa-ledger/drizzle-adapter" />
  <File name="@summa-ledger/prisma-adapter" />
  <File name="@summa-ledger/kysely-adapter" />
  <File name="@summa-ledger/memory-adapter" />
  <File name="@summa-ledger/client" />
  <File name="@summa-ledger/redis-storage" />
  <File name="@summa-ledger/telemetry" />
  <File name="@summa-ledger/test-utils" />
</Files>

| Package | Description |
|---------|-------------|
| `summa` | Main ledger library with all managers and plugins |
| `@summa-ledger/core` | Core types, adapter interface, plugin interface, SQL dialects (PostgreSQL, MySQL, SQLite) |
| `@summa-ledger/cli` | CLI for migrations, integrity checks, diagnostics, auto-install, and client codegen |
| `@summa-ledger/drizzle-adapter` | Drizzle ORM adapter |
| `@summa-ledger/prisma-adapter` | Prisma adapter |
| `@summa-ledger/kysely-adapter` | Kysely adapter |
| `@summa-ledger/memory-adapter` | In-memory adapter for testing |
| `@summa-ledger/client` | Type-safe HTTP client SDK with React, Vue, and Svelte bindings |
| `@summa-ledger/redis-storage` | Redis-backed secondary storage for rate limiting and caching |
| `@summa-ledger/telemetry` | CLI usage analytics + optional OpenTelemetry tracing integration |
| `@summa-ledger/test-utils` | Test assertions, structured test suite runner, balance and hash chain verification |
...
<Cards>
  ...
  <Card title="Multi-Tenancy" href="/docs/multi-tenancy">
    Ledger-as-namespace tenant isolation.
  </Card>
