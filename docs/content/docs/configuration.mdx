...
const summa = createSumma({
  ...
  schema: "@summa-ledger/summa",            // PostgreSQL schema (default: "@summa-ledger/summa")
...
PostgreSQL schema namespace for all Summa tables. Defaults to `"@summa-ledger/summa"`, which keeps all ledger tables isolated from your application's `public` schema.

```ts
schema: "@summa-ledger/summa"   // Default — tables in "@summa-ledger/summa"."account_balance", etc.
schema: "ledger"  // Custom — tables in "ledger"."account_balance", etc.
schema: "public"  // All tables in the public schema (not recommended)
```

All table references are automatically qualified at runtime (e.g., `"@summa-ledger/summa"."account_balance"`). The CLI `migrate push` command creates the schema with `CREATE SCHEMA IF NOT EXISTS` before applying migrations.

<Callout type="info" title="Schema naming">
  The schema name must contain only alphanumeric characters and underscores, and must start with a letter or underscore. Examples: `"@summa-ledger/summa"`, `"ledger"`, `"fin_data"`.
</Callout>
...
| `outbox_event` | [**Outbox plugin**](/docs/plugins/outbox) (`outbox-cleanup`) | Outbox owns the event delivery pipeline |
| `processed_event` | [**Outbox plugin**](/docs/plugins/outbox) (`outbox-cleanup`) | Dedup table populated by outbox processing |
| `rate_limit_log` | [**Data retention plugin**](/docs/plugins/data-retention) | Always cleaned (default: 30d) |
| `audit_log` | [**Data retention plugin**](/docs/plugins/data-retention) | Optional — configure `auditLogs` |
| `hot_account_entry` | [**Data retention plugin**](/docs/plugins/data-retention) | Optional — configure `hotAccountEntries` |
...
| `enableEventSourcing` | `boolean` | `true` | Controls event sourcing. Only disable for development/testing (see note below) |
| `enableHashChain` | `boolean` | `true` | Controls hash chain integrity. Only disable for development/testing (see note below) |
...
| `useDenormalizedBalance` | `boolean` | `true` | Store cached balance on `account_balance` for O(1) reads. See [Scaling — Denormalized Balance](/docs/scaling#denormalized-balance-cache) |
...
| `lockMode` | `"wait" \| "nowait" \| "optimistic"` | `"wait"` | Lock acquisition mode. `"nowait"` fails immediately if locked. `"optimistic"` skips `FOR UPDATE` entirely — relies on `UNIQUE(account_id, version)` constraint for conflict detection |
...
<Callout type="warn" title="enableEventSourcing and enableHashChain">
  These flags are enabled by default and control whether event sourcing and hash chain logic runs. Setting them to `false` **will** skip event/hash logic — only do this for development or testing. **Never disable in production** as it removes your immutability and tamper-detection guarantees.
</Callout>
...
| `service` | `string` | `"@summa-ledger/summa"` | Service name in structured output |
