...
The `database` field accepts any Summa adapter instance. The `currency` sets the default ISO 4217 currency code for the ledger. By default, all tables are created in the `"summa"` PostgreSQL schema — see [Configuration](/docs/configuration#schema) to customize.
