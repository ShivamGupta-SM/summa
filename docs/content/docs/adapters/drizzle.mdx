...
```bash
pnpm add @summa-ledger/drizzle-adapter drizzle-orm pg
```
...
This outputs schema-aware table definitions you can import alongside your own Drizzle schema. If you have plugins configured in your `summa.config.ts`, the generated schema will include their tables too.

When `schema` is set to a non-public value (default: `"@summa-ledger/summa"`), the generated file uses `pgSchema()` to namespace all tables:

```ts title="src/db/summa-schema.ts (generated)"
import { pgSchema, uuid, text, bigint, boolean, timestamp, jsonb, index, uniqueIndex } from "drizzle-orm/pg-core";

export const summaSchema = pgSchema("@summa-ledger/summa");

export const accountBalance = summaSchema.table("account_balance", {
  ...
  index("account_balance_holder_id_idx").on(table.holderId),
]);

// ... all other tables
```

If `schema` is set to `"public"`, the generator uses `pgTable()` instead of `pgSchema().table()`.
