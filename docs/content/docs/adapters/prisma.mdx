...
```bash
pnpm add @summa-ledger/prisma-adapter @prisma/client
```
...
When using a non-public schema (default: `"@summa-ledger/summa"`), the generated Prisma models include `@@schema("@summa-ledger/summa")` and the datasource block includes `schemas = ["@summa-ledger/summa"]`.
