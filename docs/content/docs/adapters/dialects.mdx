...
```ts
import { postgresDialect, mysqlDialect, sqliteDialect } from "@summa/core/db";
```
...
```ts
import { mysqlDialect } from "@summa/core/db";
```
...
```ts
import { sqliteDialect } from "@summa/core/db";
```
...
```ts
import { createModelResolver } from "@summa/core/db";
...
// resolver.getModelName("account_balance")  → "accounts"
// resolver.getFieldName("account_balance", "holder_id") → "user_id"
```
