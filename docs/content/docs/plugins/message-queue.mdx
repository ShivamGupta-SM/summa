...
The `@summa/message-queue` package provides a **Redis Streams-backed message bus** for high-throughput event delivery. It replaces the outbox polling pattern (20 msg/s) with push-based streaming (50K-100K msg/s) using Redis Streams — no new infrastructure required.
