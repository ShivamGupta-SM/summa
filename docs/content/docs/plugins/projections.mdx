...
The `@summa-ledger/projections` package provides a **CQRS projection runner** that maintains denormalized read models from the event stream. Projections consume events from Redis Streams and update separate tables optimized for reads — separating the read path from the write path.

This enables:
- **Read replicas** serving projection tables (no contention with writes)
- **Fast queries** on denormalized data (no joins across entry tables)
...
- Reads new events from `entry` since the last watermark
...
```ts
import type { Projection } from "@summa-ledger/projections";
