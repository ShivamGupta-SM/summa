...
The `outbox-cleanup` worker also handles cleanup of the `processed_event` deduplication table. When the outbox processes events, it records their IDs in `processed_event` to prevent duplicate delivery. These dedup records are automatically cleaned up alongside processed outbox entries, using the same `retentionHours` setting.

<Callout type="info">
  Previously, `processed_event` cleanup was handled by a separate maintenance plugin. It is now part of the outbox plugin since the outbox owns the event processing pipeline that populates this table.
</Callout>

## Type Inference

```ts
import type { OutboxStats, WebhookEndpoint, WebhookDelivery } from "summa/plugins";
