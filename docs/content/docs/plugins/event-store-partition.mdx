...
The event store partition plugin converts `entry` to a **PostgreSQL range-partitioned table**, partitioned by `created_at`. This enables partition pruning on time-range queries and keeps each partition at a manageable size — critical for ledgers with billions of entries.
...
2. Renames the existing table to `entry_old`
...
Shows whether `entry` is partitioned, lists all child partitions with estimated row counts and sizes.
