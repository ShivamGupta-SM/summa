...
import { search, pgSearchBackend } from "summa/plugins";
...
```ts
import { createSumma } from "summa";
import { search, typesenseBackend } from "summa/plugins";
...
```ts
import { search, meilisearchBackend } from "summa/plugins";
...
```ts
import type { SearchResult, ReindexStatus } from "summa/plugins";
