...
| [`batchEngine`](/docs/scaling#transaction-batching-batch-engine) | Performance | No | High-throughput transaction batching for 100,000+ TPS |
...
Plugin workers only run when their plugin is registered. Examples include outbox delivery, reconciliation checks, snapshot creation, and data retention cleanup. Each plugin's docs list its workers.
