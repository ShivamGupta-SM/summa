...
| [`statements`](/docs/plugins/statements) | Reporting | No | Account statement generation with JSON, CSV, and PDF download |
...
**Available operation types:** `account.create`, `account.freeze`, `account.unfreeze`, `account.close`, `transaction.credit`, `transaction.debit`, `transaction.transfer`, `transaction.refund`, `hold.create`, `hold.commit`, `hold.void`
