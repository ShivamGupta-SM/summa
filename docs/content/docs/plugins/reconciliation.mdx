...
The reconciliation plugin compares account balances against the sum of their entries to detect mismatches caused by bugs, race conditions, or data corruption. It performs a comprehensive **5-step verification**.
...
2. On each run, `SELECT DISTINCT account_id FROM entry WHERE created_at > $watermark` finds only active accounts
