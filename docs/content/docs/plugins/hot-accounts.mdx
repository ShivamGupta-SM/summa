...
Hot account detection is **automatic**. When an account exceeds `advanced.hotAccountThreshold` transactions per minute (default: 1000), the core transaction pipeline routes subsequent writes through the `hot_account_entry` staging table instead of directly updating the account balance.

No manual marking is needed — accounts transition to the hot path transparently based on traffic patterns.

## Realtime Balance

System account balances are updated every flush cycle (~30s). If you need the up-to-the-moment balance including unflushed hot entries, use `getRealtimeBalance()`:

```ts
import { getRealtimeBalance } from "summa/plugins";

const balance = await getRealtimeBalance(summa.$context, "@Fees");
console.log(balance.committedBalance);   // Last flushed balance
console.log(balance.pendingDelta);       // SUM of unflushed hot entries
...
```
GET /system-accounts/:identifier/realtime-balance
```
