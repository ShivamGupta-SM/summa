...
The statements plugin generates **account statements** for any date range. It combines entries and transfer data to produce line-item statements with computed summaries (opening/closing balance, totals). Supports JSON, CSV, and PDF output formats.
...
- The plugin creates a `statement_job` table for async generation. Other queries use existing `entry`, `transfer`, and `account` tables.
