...
```ts
import { getDlqStats } from "summa/plugins";
...
```ts
import { listUnresolvedEvents } from "summa/plugins";
...
```ts
import { retryEvent } from "summa/plugins";
...
Marks a failed event as resolved — it won't be retried again:

```ts
import { resolveEvent } from "summa/plugins";
...
```ts
import type { FailedEvent, DlqStats } from "summa/plugins";
