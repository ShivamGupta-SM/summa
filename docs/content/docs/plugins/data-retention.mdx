...
The data retention plugin manages data lifecycle for plugin-owned tables. Core cleanup (idempotency keys, worker leases) is handled by core workers automatically. This plugin provides configurable retention policies for audit logs, hot account entries, velocity logs, and FX quotes.
...
<Callout type="info">
  Core-owned tables (idempotency keys, worker leases) are cleaned automatically by core workers. Processed event cleanup is handled by the outbox plugin. This plugin manages retention for plugin-owned tables only.
</Callout>
...
| `hotAccountEntries` | `string` | — | Retention for flushed hot account entries |
...
```sql
DELETE FROM "@summa-ledger/summa"."rate_limit_log"
...
| `processed_event` | [Outbox plugin](/docs/plugins/outbox) (`outbox-cleanup`) | Yes |
| `rate_limit_log` | **This plugin** (always, default: 30d) | — |
| `audit_log` | **This plugin** (if `auditLogs` set) | — |
| `hot_account_entry` | **This plugin** (if `hotAccountEntries` set) | — |
| `fx_rate_quote` | **This plugin** (if `fxQuotes` set) | — |

The principle: **if core writes to a table, core cleans it up**. The outbox plugin owns `processed_event` because it populates that table during event delivery. This plugin handles the remaining plugin-owned tables.
