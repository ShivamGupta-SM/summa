...
For multi-instance deployments, configure `@summa/redis-storage` as secondary storage. This enables distributed rate limiting, shared idempotency caching, and plugin state across instances:
...
async function shutdown() {
  ...
  // 2. Disconnect Redis (if using @summa/redis-storage)
  await disconnect();
...
For production-grade distributed tracing, use the `@summa/telemetry` package which provides optional OpenTelemetry integration:
...
```ts
import { createOtelLogger, createRequestTracer } from "@summa/telemetry";
...
plugins: [
  ...
  maintenance(),                    // Cleanup
  dataRetention(),                  // Unified data lifecycle management
