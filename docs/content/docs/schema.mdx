...
Summa creates and manages its own database tables. All tables live in a configurable PostgreSQL schema (default: `"@summa-ledger/summa"`), keeping them isolated from your application tables.

```ts
const summa = createSumma({
  database: adapter,
  schema: "@summa-ledger/summa", // PostgreSQL schema name (default)
});
```
...
Stores tenant/namespace metadata for multi-tenancy. Each ledger is an isolated namespace with its own accounts, transactions, and system accounts.
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `ledger_id` | UUID | FK to `ledger` — scopes events per tenant |
...
| `overdraft_limit` | BIGINT | Max negative balance allowed in minor units. 0 = unlimited when overdraft enabled. Enforced on debits, transfers, and holds. |
...
**Key indexes:** `(ledger_id, holder_id, currency)` unique, `(account_type)`, `(parent_account_id)`
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `ledger_id` | UUID | FK to `ledger` — each ledger has its own system accounts |
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `ledger_id` | UUID | FK to `ledger` — scopes transactions per tenant |
...
account_balance → account_balance_version (append-only state)
    ...
    └── account_transaction_log          └── outbox → processed_event
