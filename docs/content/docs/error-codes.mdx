...
```ts
import { SummaError } from "summa/error";
...
| `VALIDATION_ERROR` | 400 | Request body validation failed (missing or invalid field type) |
...
import { BASE_ERROR_CODES } from "summa/error";
```
