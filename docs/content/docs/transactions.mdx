...
A **balancing debit** automatically caps the amount to the account's available balance. If the account has less than the requested amount, the transaction succeeds with the available balance instead of failing with `INSUFFICIENT_BALANCE`.

<Tabs items={["Balancing Debit", "Balancing Transfer"]}>
<Tab value="Balancing Debit">

```ts
// Account has $50.00 available
const txn = await summa.transactions.debit({
  holderId: "user_123",
  amount: 100_00,        // Requested $100
  reference: "sweep-001",
  balancing: true,
  idempotencyKey: "sweep_001",
});
// txn.amount === 50_00 — capped to available balance
// txn.metadata === { balancing: true, requestedAmount: 100_00 }
```

</Tab>
<Tab value="Balancing Transfer">

```ts
// Source account has $30.00 available
const txn = await summa.transactions.transfer({
  sourceHolderId: "user_123",
  destinationHolderId: "treasury",
  amount: 100_00,        // Requested $100
  reference: "sweep-002",
  balancing: true,
  idempotencyKey: "sweep_002",
});
// txn.amount === 30_00 — capped to source's available balance
```
...
**Use cases:**
- **Account sweeps**: Move all available funds to a treasury account without querying the balance first
- **Best-effort payments**: Pay as much as possible toward a bill
- **Account closure**: Drain remaining balance during account close

<Callout type="info" title="Zero-amount transactions">
  If the available balance is zero, a balancing debit/transfer succeeds with amount `0`. The original requested amount is always stored in `metadata.requestedAmount`.
</Callout>

### Force Debit

A privileged debit that **bypasses all balance and overdraft checks**. Use for chargebacks, network-initiated adjustments, regulatory debits, or any case where the debit must succeed regardless of account balance.

```ts
const txn = await summa.transactions.forceDebit({
  holderId: "merchant_1",
  amount: 500_00,
  reference: "chargeback-001",
  reason: "Visa chargeback — dispute lost",
  destinationSystemAccount: "@Chargebacks",
  idempotencyKey: "cb_001",
});
```

<TypeTable
  type={{
    holderId: { type: "string", description: "Holder ID of the account to debit.", required: true },
    ...
    reason: { type: "string", description: "Mandatory reason for the forced debit (stored in metadata as `forceReason`).", required: true },
    destinationSystemAccount: { type: "string", description: "System account to receive the funds.", default: "@World" },
    idempotencyKey: { type: "string", description: "Idempotency key for safe retries." },
  }}
/>

<Callout type="warn" title="Use with caution">
  `forceDebit` can push an account balance arbitrarily negative. Restrict access to this operation in your application — it should only be used by trusted system processes, not exposed to end users.
</Callout>

### Force Transfer

A privileged transfer that **bypasses all balance and overdraft checks** on the source account. Use for clawbacks, regulatory seizures, or inter-account forced moves where the transfer must succeed regardless of source balance.

```ts
const txn = await summa.transactions.forceTransfer({
  sourceHolderId: "merchant_1",
  destinationHolderId: "escrow_account",
  amount: 1000_00,
  reference: "clawback-001",
  reason: "Regulatory seizure — compliance order #456",
...
<TypeTable
  type={{
    sourceHolderId: { type: "string", description: "Holder ID of the source account.", required: true },
    destinationHolderId: { type: "string", description: "Holder ID of the destination account.", required: true },
    amount: { type: "number", description: "Amount in smallest currency units.", required: true },
    reference: { type: "string", description: "Business reference.", required: true },
    reason: { type: "string", description: "Mandatory reason for the forced transfer (stored in metadata as `forceReason`).", required: true },
    exchangeRate: { type: "number", description: "Exchange rate for cross-currency transfers (rate × 1,000,000)." },
    idempotencyKey: { type: "string", description: "Idempotency key for safe retries." },
  }}
/>

<Callout type="warn" title="Use with caution">
  `forceTransfer` can push the source account balance arbitrarily negative. Restrict access to this operation — it should only be used by trusted system processes.
</Callout>
...
<Callout type="warn" title="Field-by-field validation">
  A duplicate key with different parameters throws a `CONFLICT` error (HTTP 409) that tells you **exactly which field** changed — e.g., `"Idempotency key 'X' already used with different amount"`. See [Error Codes — Idempotency](/docs/error-codes#idempotency--field-by-field-validation) for details.
</Callout>
