...
A **balancing debit** automatically caps the amount to the account's available balance — inspired by [TigerBeetle's `balancing_debit` flag](https://docs.tigerbeetle.com/). If the account has less than the requested amount, the transaction succeeds with the available balance instead of failing with `INSUFFICIENT_BALANCE`.
...
<TypeTable
  type={{
    ...
    reason: { type: "string", description: "Mandatory reason for the forced debit (stored in metadata as _forceDebitReason).", required: true },
