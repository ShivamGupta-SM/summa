...
This document describes Summa's immutability architecture. Core financial tables use an **append-only design** — every state change creates a new row instead of updating in place. This matches the immutability guarantees of purpose-built financial databases like TigerBeetle, while retaining the flexibility of PostgreSQL.
...
How Summa's architecture compares to other financial ledger systems:

| Feature | TigerBeetle | Formance | Blnk | Summa |
|---------|:-:|:-:|:-:|:-:|
| Transfers immutable | Yes | Yes | Yes | **Yes** |
| Balance append-only | Derived from transfers | Computed on read | No (mutable) | **Yes** (version table) |
| Status tracking | New transfer record | N/A | Separate record | **Append-only log** |
| Corrections | Reverse transfer | Reverse posting | Reverse entry | **New status row** |
| DB-level enforcement | Custom DBMS | Immutable log | None | **Triggers + RLS** |
| Cryptographic proof | Hash chain | Immutable log | None | **Merkle tree** |
| Individual event proof | N/A | N/A | N/A | **O(log n)** |
| Transient error classification | Yes | No | No | **Yes** |
| Field-by-field idempotency | Yes | No | No | **Yes** |
| Balancing debit/transfer | Yes | No | No | **Yes** |

<Callout type="info" title="Design Philosophy">
  TigerBeetle achieves immutability by building a custom database where UPDATE and DELETE operations simply don't exist. Summa achieves the same guarantee on PostgreSQL through a combination of append-only table design, database triggers, row-level security, and cryptographic verification — making it the most secure approach possible on a general-purpose database.
</Callout>

### TigerBeetle-Inspired Features

The following features are directly inspired by TigerBeetle's design patterns:
