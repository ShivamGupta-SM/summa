...
description: Summa's integrity architecture — entries are the immutable source of truth, balances are mutable with checksum protection.
icon: Map
---

## Overview

This document describes Summa's integrity architecture. The design follows a TigerBeetle-inspired philosophy: **entries are the immutable source of truth**, while balances are mutable projections protected by HMAC checksums and optimistic locking.

<Callout type="info" title="v2 Architecture">
  Summa v2 simplified the data model from 17 core tables to 10. The append-only version tables (`account_balance_version`, `system_account_version`, `transaction_status`) were removed. Instead, the `entry` table serves as both the double-entry journal and the tamper-proof event log (with per-account hash chains). The `account` table has mutable balance fields protected by HMAC checksums. The historical content below documents the v1 approach for reference — v2 achieves the same security guarantees with fewer tables.
</Callout>
...
const IMMUTABLE_MODELS = new Set([
  'entry', 'block_checkpoint', 'merkle_node',
  'entity_status_log',
]);
```
...
CREATE POLICY entry_insert ON entry FOR INSERT WITH CHECK (true);
