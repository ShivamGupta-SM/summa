...
4. **Plugins** — multiselect from 25 built-in plugins (audit log, reconciliation, snapshots, velocity limits, hold expiry, freeze expiry, outbox, DLQ manager, hot accounts, scheduled transactions, maintenance, admin, OpenAPI, observability, statements, i18n, MCP, and more)
...
- Event hash chain per aggregate (samples up to 50 aggregates; warns if the total aggregate count exceeds the sample size)
