...
Generate SQL for converting the `entry` table to a PostgreSQL range-partitioned table. Does NOT auto-execute — writes a SQL file for manual review.
...
Show current partition status for the `entry` table.
...
Displays:
- Whether `entry` is partitioned (checks `pg_class.relkind`)
...
- Verifies that `trg_immutable_*` triggers exist on all immutable core tables (`entry`, `block_checkpoint`, `merkle_node`, `entity_status_log`)
...
| **Core tables** | `account`, `transfer`, `entry`, `block_checkpoint` |
