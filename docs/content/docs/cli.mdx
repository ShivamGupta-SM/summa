...
If your config uses a non-public schema (default: `"@summa-ledger/summa"`), the command automatically runs `CREATE SCHEMA IF NOT EXISTS` before applying migrations. All tables are created in the configured schema namespace.
