...
Summa provides first-class testing support through the **memory adapter** (no database required) and the **`@summa-ledger/test-utils`** package with assertion helpers for balance verification, double-entry integrity, and hash chain validation.

## Setup

```bash
pnpm add -D @summa-ledger/memory-adapter @summa-ledger/test-utils vitest
```
...
The `@summa-ledger/test-utils` package provides assertion helpers:
...
```ts
import { assertAccountBalance } from "@summa-ledger/test-utils";
...
```ts
import { assertDoubleEntry } from "@summa-ledger/test-utils";
...
```ts
import { assertHashChain } from "@summa-ledger/test-utils";
