...
|--------|---------|-------------|
| [`outbox`](/docs/plugins/outbox) | `outbox-processor` (5s), `outbox-cleanup` (6h), `webhook-delivery-processor` (5s), `webhook-delivery-cleanup` (1d) | Event delivery, outbox cleanup, webhook delivery |
...
INSERT INTO "summa"."worker_lease" (worker_id, lease_holder, lease_until)
