...
> **This page documents `summa/webhooks`** — a standalone utility for **signing and verifying** webhook payloads. It does not send or receive webhooks on its own.
>
> Looking for something else?
> - **Automatic HTTP delivery & event bus** → [Outbox plugin](/docs/plugins/outbox) (supports both publisher functions and managed webhook delivery)
...
import type {
  ...
  WebhookPayload,
} from "@summa-ledger/summa/webhooks";
```
...
```ts
import { WebhookError } from "@summa-ledger/summa/webhooks";
...
app.post("/webhooks/summa", (req, res) => {
  ...
  try {
    ...
    if (error instanceof WebhookError) {
      // error.code: "INVALID_SIGNATURE" — HMAC mismatch
      //             "PAYLOAD_EXPIRED"   — timestamp exceeds tolerance
      console.error(`Webhook rejected: ${error.code}`);
    }
    res.sendStatus(401);
  }
});
...
The `WebhookError` class provides a typed `code` property for programmatic error handling:

| Code | Meaning |
|------|---------|
| `INVALID_SIGNATURE` | HMAC-SHA256 signature does not match the payload |
| `PAYLOAD_EXPIRED` | Payload timestamp exceeds the configured tolerance |
...
The outbox plugin has **built-in webhook delivery** — enable `webhooks` mode and users register endpoints via the API. Payloads are signed with HMAC-SHA256 automatically:
...
You can also use the publisher mode with `summa/webhooks` for custom delivery logic:
